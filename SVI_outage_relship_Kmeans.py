# -*- coding: utf-8 -*-
"""
Created on Sat May  4 07:39:15 2024
...
data_for_clustering = data[columns_for_clustering].copy()

# Replace NaN values with the mean of each column for numeric data only
for col in data_for_clustering.columns:
    data_for_clustering.loc[:, col] = data_for_clustering[col].fillna(data_for_clustering[col].mean())

# Check for and handle non-numeric values
for col in data_for_clustering.columns:
    try:
        data_for_clustering[col] = pd.to_numeric(data_for_clustering[col])
    except ValueError:
...
sns.pairplot(data[columns_for_clustering + ['Cluster']], hue='Cluster', vars=columns_for_clustering)
plt.show()
...
data.to_excel('Vulnerability K_cluster.xlsx', index=False)
